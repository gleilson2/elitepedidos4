...
const ProductsPanel: React.FC = () => {
  ...
  useEffect(() => {
    const loadProductImages = async () => {
      ...
      if (!supabaseUrl || !supabaseKey || 
          supabaseUrl === 'https://afceshaeqqmbrtudlhwz.supabase.co/rest/v1/pesagem_temp' || 
  ...
  const handleRefreshProducts = async () => {
    try {
      ...
      // Debug: Verificar produtos no banco vs código
      await debugProductsInDatabase();
    } catch (error) {
      console.error('Erro ao recarregar produtos:', error);
    }
  };

  const debugProductsInDatabase = async () => {
    try {
      console.log('🔍 DEBUG: Verificando produtos no banco vs código...');
      
      // Importar produtos do código
      const { products: codeProducts } = await import('../../data/products');
      
      console.log('📊 Estatísticas:');
      console.log(`- Produtos no código: ${codeProducts.length}`);
      console.log(`- Produtos no banco: ${products.length}`);
      
      // Verificar quais produtos do código não estão no banco
      const missingInDatabase = codeProducts.filter(codeProduct => 
        !products.some(dbProduct => dbProduct.id === codeProduct.id)
      );
      
      if (missingInDatabase.length > 0) {
        console.warn('⚠️ Produtos do código que NÃO estão no banco:');
        missingInDatabase.forEach(product => {
          console.warn(`- ${product.name} (ID: ${product.id})`);
        });
        
        // Perguntar se quer migrar
        if (confirm(`Encontrados ${missingInDatabase.length} produtos do código que não estão no banco.\n\nDeseja migrar estes produtos para o banco de dados?`)) {
          await migrateProductsToDatabase(missingInDatabase);
        }
      } else {
        console.log('✅ Todos os produtos do código estão no banco de dados');
      }
      
      // Verificar produtos no banco que não estão no código
      const extraInDatabase = products.filter(dbProduct => 
        !codeProducts.some(codeProduct => codeProduct.id === dbProduct.id)
      );
      
      if (extraInDatabase.length > 0) {
        console.log('ℹ️ Produtos no banco que não estão no código:');
        extraInDatabase.forEach(product => {
  ...
  const migrateProductsToDatabase = async (productsToMigrate: any[]) => {
    try {
      console.log('🚀 Iniciando migração de produtos para o banco...');
      ...
      successMessage.innerHTML = `
        ...
        Produtos migrados para o banco com sucesso!
      `;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end mb-4">
        <div className="flex gap-2">
          <button
            onClick={debugProductsInDatabase}
